...
class ReportGenerationService {
  ...
  async generateProgressReport(childId, period = '30d') {
    try {
      const response = await api.post('/api/reports/generate/progress', {
        child_id: childId,
        period: period
      });
  ...
  async generateSummaryReport(childId) {
    try {
      const response = await api.post('/api/reports/generate/summary', {
        child_id: childId
      });
      return response.data;
    } catch (error) {
  ...
  async generateProfessionalReport(childId, professionalId) {
    try {
      const response = await api.post('/api/reports/generate/professional', {
        child_id: childId,
  ...
  async getAvailableChildren() {
    try {
      const response = await api.get('/api/children/active');
      return response.data;
    } catch (error) {
