...
function Invoke-ApiTest {
    ...
    try {
        ...
        if ($statusCode -ge 200 -and $statusCode -lt 300) {
            ...
            Write-Host "   WARNING ($statusCode)" -ForegroundColor Yellow
        }
...
$healthResult = Invoke-ApiTest -Method "GET" -Endpoint "/health" -Description "Backend Health Check"
...
$authToken = $null
if ($regResult.Success -and $regResult.StatusCode -eq 201) {
    $loginData = @{
        email = "test.parent@email.com"
        password = "TestPassword123!"
    }
...
# 4. REPORTS ANALYTICS  
Write-Host "4. TESTING REPORTS AND ANALYTICS" -ForegroundColor Yellow
Write-Host "---------------------------------" -ForegroundColor Yellow
...
if ($profResult.Success -and $profResult.StatusCode -eq 201) {
    $profLoginData = @{
        email = "test.doctor@clinic.com"
        password = "DoctorPass123!"
    }
    ...
    Write-Host "   SKIPPED: Professional registration failed" -ForegroundColor Yellow
    Write-Host ""
}
