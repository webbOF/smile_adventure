#!/usr/bin/env python3
"""
Fixed Mock Backend Server for Frontend Integration Testing
"""
...
def create_access_token(data: dict):
    ...
    # Ensure subject is a string for JWT compatibility
    if "sub" in to_encode:
...
def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)):
    try:
        print(f"Verifying token: {credentials.credentials[:20]}...")  # Debug log
        payload = jwt.decode(credentials.credentials, SECRET_KEY, algorithms=[ALGORITHM])
        user_id_str: str = payload.get("sub")
        print(f"Token payload user_id (string): {user_id_str}")  # Debug log
        if user_id_str is None:
            print("❌ Token missing user_id")  # Debug log
            raise HTTPException(status_code=401, detail="Invalid token")
        
        # Convert string user_id back to integer
        user_id = int(user_id_str)
        if user_id not in mock_users:
            print(f"❌ User {user_id} not found in mock_users")  # Debug log
